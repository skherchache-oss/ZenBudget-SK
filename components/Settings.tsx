...
interface SettingsProps { 
  ...
  onLogout: () => void; 
  onShowWelcome: () => void; 
  onBackup: () => void; // Ajouté pour le backup JSON
  onImport: (file: File) => void; // Ajouté pour l'import JSON
} 
...
const Settings: React.FC<SettingsProps> = ({ state, onUpdateAccounts, onSetActiveAccount, onDeleteAccount, onReset, onShowWelcome, onBackup, onImport }) => { 
  ...
  return ( 
    <div className="space-y-6 pb-32 overflow-y-auto no-scrollbar h-full px-4 pt-6 fade-in"> 
        ...
        <div className="bg-white rounded-[24px] border border-slate-50 overflow-hidden shadow-sm"> 
          ...
          <button onClick={handleExportCSV} className="w-full flex items-center justify-between p-4 hover:bg-slate-50 transition-colors"> 
            <div className="flex items-center gap-3"> 
              ...
              <span className="text-[10px] font-black uppercase tracking-widest text-slate-700">Export complet (CSV / Excel)</span> 
            </div> 
            <svg className="w-3 h-3 text-slate-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}><path d="M9 5l7 7-7 7" /></svg> 
          </button> 
        </div> 
      ...
      <section>
        <SectionTitle title="Sauvegarde Cloud (JSON)" />
        <div className="bg-white rounded-[24px] border border-slate-50 overflow-hidden shadow-sm">
          <button onClick={onBackup} className="w-full flex items-center justify-between p-4 hover:bg-slate-50 transition-colors border-b border-slate-50">
            <div className="flex items-center gap-3">
              ...
              <span className="text-[10px] font-black uppercase tracking-widest text-indigo-600">Exporter ma sauvegarde</span>
            </div>
          </button>
          ...
          <button onClick={() => fileInputRef.current?.click()} className="w-full flex items-center justify-between p-4 hover:bg-slate-50 transition-colors">
            <div className="flex items-center gap-3">
              ...
              <span className="text-[10px] font-black uppercase tracking-widest text-amber-600">Importer une sauvegarde</span>
            </div>
          </button>
        </div>
      </section>
