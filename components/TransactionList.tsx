  ...
  return (
    <div className="flex items-center bg-white first:rounded-t-[20px] last:rounded-b-[20px] relative overflow-hidden h-[60px]">
      <div className={`absolute inset-y-0 right-0 flex transition-transform duration-500 ease-[cubic-bezier(0.23,1,0.32,1)] z-20 ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <button onClick={() => onEdit(t)} className="w-20 h-full bg-indigo-600 text-white flex flex-col items-center justify-center gap-1">
        ...
        <button onClick={() => { if(isConfirmingDelete) { onDelete(t.id); } else { setIsConfirmingDelete(true); } }} className={`w-20 h-full flex flex-col items-center justify-center gap-1 transition-all ${isConfirmingDelete ? 'bg-black text-white' : 'bg-red-600 text-white'}`}>
      ...
      <div 
        ...
        style={{ transform: `translateX(${isOpen ? -160 : 0}px)` }}
        onClick={onToggle}
      >
                ...
                {Array.from({ length: daysInMonth }).map((_, i) => {
                  ...
                  return (
                    <button key={day} onClick={() => onSelectDay(day)}
                      className={`h-14 rounded-[16px] flex flex-col items-center justify-between py-2 border ${isSelected ? 'bg-slate-900 border-slate-900 text-white z-10' : 'bg-white border-slate-50'}`}
                    >
