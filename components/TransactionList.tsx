...
const TransactionItem: React.FC<{ 
  ...
  return (
    <div className="flex items-center bg-white first:rounded-t-[20px] last:rounded-b-[20px] relative overflow-hidden h-[64px]">
      ...
      <div 
        ...
        onClick={onToggle}
      >
        <div className={`w-9 h-9 rounded-xl flex items-center justify-center text-lg shrink-0 ${isVirtual ? 'bg-amber-50 border border-amber-100' : 'bg-gray-50'}`}>
          {category?.icon || '❓'}
        </div>
        <div className="flex-1 min-w-0">
          ...
          <div className="text-[9px] text-gray-400 truncate mt-0.5">{t.comment || 'Note vide'}</div>
        </div>
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between px-1">
        ...
        <div className="bg-white border border-slate-100 rounded-full px-3 py-1.5 flex items-center gap-2 shadow-sm ring-2 ring-indigo-50/50">
           <span className="text-[8px] font-black uppercase tracking-widest text-slate-400">Solde actuel</span>
      ...
      <div className="flex bg-slate-200/40 p-1 rounded-xl border border-slate-100/50 shrink-0">
        <button onClick={() => setViewMode('CALENDAR')} className={`flex-1 py-2 text-[9px] font-black uppercase tracking-widest rounded-lg transition-all ${viewMode === 'CALENDAR' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-400'}`}>Calendrier</button>
        <button onClick={() => setViewMode('LIST')} className={`flex-1 py-2 text-[9px] font-black uppercase tracking-widest rounded-lg transition-all ${viewMode === 'LIST' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-400'}`}>Liste</button>
      </div>
      ...
      {viewMode === 'CALENDAR' ? (
        <div className="space-y-4">
          <div className="bg-white rounded-[28px] p-4 shadow-sm border border-slate-100">
            <div className="grid grid-cols-7 mb-2">
              {['L', 'M', 'M', 'J', 'V', 'S', 'D'].map((d, i) => (
                <div key={i} className="text-center text-[8px] font-black uppercase text-slate-300 py-1">{d}</div>
              ))}
            </div>
              ...
              {Array.from({ length: daysInMonth }).map((_, i) => {
                ...
                return (
                  <button 
                    ...
                    className={`h-14 rounded-xl flex flex-col items-center justify-between py-1.5 transition-all border relative ${selectedDay === day ? 'bg-slate-900 border-slate-900 text-white shadow-lg z-10 scale-105' : 'bg-slate-50/30 border-slate-100 hover:bg-slate-100'}`}
                  >
                    <span className="text-[10px] font-black">{day}</span>
                    <div className="flex flex-col items-center">
                      <span className={`text-[7px] font-black tracking-tighter ${selectedDay === day ? 'text-white/90' : (balance >= 0 ? 'text-indigo-600' : 'text-red-500')}`}>
                        {Math.round(balance)}
                      </span>
          ...
          <div className="space-y-2">
            <h3 className="text-[8px] font-black uppercase tracking-widest text-slate-400 ml-1">Détail du {selectedDay}</h3>
