...
const Dashboard: React.FC<DashboardProps> = ({ 
  ...
  const fetchAiAdvice = async () => {
    ...
    try {
      // Utilisation de FETCH direct sur l'API V1 (Stable) au lieu de V1BETA
      const response = await fetch(
        `https://generativelanguage.googleapis.com/v1/models/gemini-1.5-flash:generateContent?key=${apiKey}`,
        {
          ...
          body: JSON.stringify({
            contents: [{
              parts: [{
                text: `Tu es un coach financier Zen. Donne un conseil très court (max 60 car.) en français avec un emoji. Pas de guillemets. Variation: ${Math.random()}`
              }]
            }]
          })
        }
      ...
      setAiAdvice("ZenTip : La régularité est la clé de la sérénité financière. 🌿"); 
    } finally { 
  ...
  const handleExportCSV = () => {
    try {
      ...
      link.href = url; link.download = `ZenBudget_Export.csv`; link.click();
