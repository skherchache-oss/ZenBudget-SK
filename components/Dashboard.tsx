...
// Utilisation du SDK officiel Google Generative AI
...
const Dashboard: React.FC<DashboardProps> = ({ 
  ...
  // Récupération du conseil IA (Gemini)
  const fetchAiAdvice = async () => {
    // Utilisation de import.meta.env pour Vite (Vercel)
    const apiKey = (import.meta as any).env.VITE_GEMINI_API_KEY || "";
    
    if (!apiKey) {
      setAiAdvice("ZenTip : Optimisez vos charges fixes pour augmenter votre épargne résiduelle.");
      return;
    }
    ...
    try {
      ...
      const prompt = `Tu es un expert en finance personnelle pour l'application ZenBudget.
        Contexte :
        - Solde disponible (après charges fixes) : ${availableBalance}€
        - Dépenses totales du mois : ${stats.expenses}€
        - Revenus identifiés : ${stats.income}€
        
        MISSION :
        Donne un conseil financier pragmatique TRÈS COURT (max 60 car).
  ...
  // Déclenchement automatique au chargement (avec délai pour éviter le spam)
  useEffect(() => {
    ...
    <div className="flex flex-col h-full space-y-6 overflow-y-auto no-scrollbar pb-32 px-1 fade-in">
      ...
      <div className="flex items-center justify-between pt-6">
        ...
        >
          ...
          <span className="text-[10px] font-black uppercase tracking-widest whitespace-nowrap">Exporter</span>
        </button>
      </div>
