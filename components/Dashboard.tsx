...
const Dashboard: React.FC<DashboardProps> = ({ 
  ...
  const fetchAiAdvice = async () => {
    // 1. Vérifie si tu utilises VITE_ ou un autre préfixe dans ton .env
    ...
    try {
      ...
      const model = genAI.getGenerativeModel({ 
        ...
        generationConfig: {
          temperature: 1, // On monte la température à 1 pour un max de variation
          topP: 0.95,
          maxOutputTokens: 40,
        }
      });
      ...
      const prompt = `Tu es un coach financier Zen. 
        Données : Solde ${availableBalance}€, Dépenses ${stats.expenses}€.
  ...
  }, [activeAccount.id]); // Se relance au changement de compte
  ...
  const handleExportCSV = () => {
    try {
      ...
      const rows: string[] = [
        ...
        "",
        `Solde Actuel${s}${f(checkingAccountBalance)} €`,
        "",
