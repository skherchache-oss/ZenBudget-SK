    ...
    <div className="flex flex-col h-full space-y-6 overflow-y-auto no-scrollbar pb-24 px-1 animate-in fade-in duration-700">
      ...
      <div className="flex items-center justify-between pt-2">
        <div className="flex flex-col">
          ...
          <p className="text-[10px] font-black uppercase tracking-widest text-indigo-500 mt-1 whitespace-nowrap overflow-hidden text-ellipsis max-w-[120px]">
        ...
        <div className="flex items-center gap-2">
          <button 
            ...
            className="px-3 py-2.5 bg-white rounded-2xl border border-slate-100 shadow-sm text-[9px] font-black uppercase tracking-widest text-slate-500 active:scale-95 transition-all hover:text-indigo-600 hover:border-indigo-100"
          >
            Export CSV
          </button>
      ...
      {/* 2. Solde Bancaire Card - Correction Rognage & Taille */}
      <div className="bg-slate-900 px-6 py-8 rounded-[40px] shadow-2xl relative overflow-visible ring-1 ring-white/10 group min-h-[140px] flex flex-col justify-center">
        <div className="relative z-10 flex flex-col gap-1">
          <div className="flex justify-between items-center mb-1">
            ...
            <div className="bg-white/10 px-2 py-1 rounded-full backdrop-blur-md">
              <span className="text-[8px] font-black text-white/60 uppercase tracking-widest">Actuel</span>
            </div>
          </div>
          <div className="flex items-baseline gap-1.5 w-full flex-wrap">
            <span className="text-4xl xs:text-5xl sm:text-6xl font-black tracking-tighter text-white leading-tight">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-indigo-600 p-5 rounded-[32px] shadow-xl shadow-indigo-100 flex flex-col gap-2 relative overflow-hidden">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-white p-5 rounded-[28px] border border-slate-50 shadow-sm flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <div className="text-[13px] font-black text-slate-800 truncate">{Math.round(stats.fixed).toLocaleString('fr-FR')}€</div>
          </div>
        </div>
        <div className="bg-white p-5 rounded-[28px] border border-slate-50 shadow-sm flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <div className="text-[13px] font-black text-slate-800 truncate">{Math.round(stats.variable).toLocaleString('fr-FR')}€</div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-slate-100 p-5 rounded-[28px] flex items-center gap-4 border border-white">
        ...
        <div className="flex-1 min-w-0">
          <p className={`text-[11px] font-medium leading-tight text-slate-600 italic transition-opacity duration-700 ${loadingAdvice ? 'opacity-30' : 'opacity-100'}`}>
            "{aiAdvice}"
          </p>
        </div>
      </div>
      ...
      <div className="bg-white p-6 rounded-[40px] border border-slate-100 shadow-sm space-y-6">
        <div className="flex items-center justify-between px-1">
          <h3 className="text-[10px] font-black uppercase tracking-[0.15em] text-slate-400">Dépenses</h3>
          <span className="text-[9px] font-black text-indigo-600 bg-indigo-50 px-2 py-1 rounded-xl uppercase">Analyse</span>
        </div>
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
            {hoveredCategory ? (
              <div className="text-center animate-in zoom-in duration-300">
                ...
                <div className="text-[11px] font-black text-slate-900 uppercase mt-0.5">{Math.round(hoveredCategory.percent)}%</div>
              </div>
            ) : (
              <div className="text-center">
                <span className="text-[9px] font-black text-slate-300 uppercase block tracking-tighter">Total</span>
        ...
        <div className="space-y-2 pt-4 border-t border-slate-50">
          {categorySummary.length > 0 ? categorySummary.slice(0, 5).map((cat, idx) => (
            <div 
              key={cat.id} 
              className={`flex items-center gap-3 p-3 rounded-[24px] transition-all ${activeIndex === idx ? 'bg-slate-50 scale-[1.02]' : 'hover:bg-slate-50/50'}`}
            ...
            <div className="py-10 text-center text-[10px] font-black text-slate-300 uppercase tracking-widest italic text-slate-400">Aucune dépense</div>
          )}
        </div>
      </div>
    </div>
