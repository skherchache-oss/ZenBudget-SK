...
const Dashboard: React.FC<DashboardProps> = ({ 
  ...
  // Appel IA robuste via Fetch (évite les bugs d'import SDK au build)
    ...
    try {
      const response = await fetch(
        `https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${apiKey}`,
        {
          ...
          body: JSON.stringify({
            contents: [{ parts: [{ text: `Conseil budget court (60 car max) pour ${availableBalance}€ dispos. Style zen.` }] }]
          })
        }
      );
      ...
      setAiAdvice("ZenTip : Restez serein et suivez vos dépenses fixes."); 
    } finally { 
