...
const App: React.FC = () => {
  ...
  // On utilise un useRef pour éviter de sauvegarder au tout premier chargement
  ...
  useEffect(() => {
    ...
    // Debounce : on attend 1 seconde de calme avant d'écrire dans le localStorage
  ...
  // On stabilise "maintenant" pour éviter les changements à la milliseconde
  const now = useMemo(() => {
    const d = new Date();
    d.setHours(12, 0, 0, 0); // On fixe l'heure pour éviter les bugs de calcul
    return d;
  }, [currentMonth, currentYear]);

  // --- FONCTION DE CALCUL DE SOLDE (COEUR DU MOTEUR) ---
  const getBalanceAtDate = (targetDate: Date, includeProjections: boolean) => {
    if (!activeAccount) return 0;
    
    // 1. Solde des transactions réelles déjà saisies
    let balance = activeAccount.transactions.reduce((acc, t) => {
      const tDate = new Date(t.date);
      return tDate <= targetDate ? acc + (t.type === 'INCOME' ? t.amount : -t.amount) : acc;
    }, 0);

    // 2. Ajout des projections (Charges fixes non encore pointées)
    if (includeProjections && targetDate > now) {
      ...
      // On calcule sur une plage de mois limitée pour éviter les boucles infinies
      let cursor = new Date(now.getFullYear(), now.getMonth(), 1);
      // On ne projette jamais au-delà de 24 mois par sécurité
      const limit = new Date(targetDate.getFullYear(), targetDate.getMonth() + 1, 1);

      while (cursor < limit && cursor < new Date(now.getFullYear() + 2, now.getMonth(), 1)) {
  ...
  // --- MÉMOISATION DES VALEURS DU DASHBOARD ---
      ...
      <header className="bg-white/80 backdrop-blur-xl border-b border-slate-100 px-4 py-3 safe-top shrink-0 z-50">
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-1 bg-slate-100 p-1 rounded-2xl border border-slate-200 shadow-sm flex-1 max-w-[200px] justify-between">
             ...
             <div className="flex items-baseline gap-1 whitespace-nowrap overflow-hidden px-1">
