...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (!firstLaunch) {
      ...
      if (diffDays > 3) {
        // Un petit délai pour ne pas agresser au démarrage
  ...
  // --- LOGIQUE DE CALCULS (Simplifiée pour la lisibilité ici) ---
  const activeAccount = useMemo(() => {
    return state.accounts.find(a => a.id === state.activeAccountId) || state.accounts[0];
  }, [state.accounts, state.activeAccountId]);

  // Les fonctions getProjectedBalanceAtDate, checkingAccountBalance, etc. 
  // doivent rester ici comme dans ton code original.
  // (Je les omets pour rester concis, mais garde les tiennes !)
  ...
  const handleUpsertTransaction = (t: any) => { /* Ta logique de save */ };
  const handleDeleteTransaction = (id: string) => { /* Ta logique delete */ };
  ...
  return (
    <div className="flex flex-col h-screen bg-[#F8F9FD] text-slate-900 overflow-hidden font-sans">
      <header className="bg-white/80 backdrop-blur-xl border-b border-slate-100 px-4 py-3 safe-top shrink-0 z-50">
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-1 bg-slate-100 p-1 rounded-2xl border border-slate-200 shadow-sm flex-1 max-w-[180px] justify-between">
             <button onClick={() => handleMonthChange(-1)} className="p-2 text-slate-400 active:scale-90"><svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}><path d="M15 19l-7-7 7-7" /></svg></button>
             <span className="text-[10px] font-black uppercase tracking-widest text-indigo-700">{MONTHS_FR[currentMonth]} {currentYear}</span>
             <button onClick={() => handleMonthChange(1)} className="p-2 text-slate-400 active:scale-90"><svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}><path d="M9 5l7 7-7 7" /></svg></button>
          </div>
        </div>
      </header>
      ...
      {/* BOUTON PLUS : On s'assure qu'il est sous la modale d'avis (z-40) */}
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-md border-t border-slate-100 flex justify-around items-center pt-2 pb-[max(1.5rem,env(safe-area-inset-bottom))] px-6 z-50">
      ...
      {/* MODAL RATING ZEN CORRIGÉE POUR NE PAS ÊTRE ROGNÉE */}
      {showRatingModal && (
        <div className="fixed inset-0 z-[100] flex items-end justify-center bg-slate-900/40 backdrop-blur-[2px] animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-md rounded-t-[40px] shadow-2xl p-8 relative animate-in slide-in-from-bottom duration-500 pb-[calc(2rem+90px)]">
            <div className="flex flex-col items-center text-center space-y-4">
              ...
              <div className="flex flex-col w-full gap-3 pt-4">
                <button 
                  onClick={() => { localStorage.setItem('zenbudget_has_rated', 'true'); setShowRatingModal(false); window.open('#', '_blank'); }}
